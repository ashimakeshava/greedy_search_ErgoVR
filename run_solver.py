...
def run_script(use_multiprocessing, n_tries_per_core, task):
    ...
    with open('collected_depths_new/task{}.pickle'.format(task), 'wb') as handle:
        pickle.dump(overall_results, handle, protocol=pickle.HIGHEST_PROTOCOL)

    # Remove temporary files
    print('\nRemoving temporary files:')
    for filename in os.listdir('collected_depths_new'):
        if 'TEMP' in filename:
            print(filename)
            os.remove('collected_depths_new/'+filename)
...
if __name__ == '__main__':
    ...
    os.makedirs('collected_depths_new/', exist_ok=True)
